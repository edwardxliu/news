...
export let curPageIndex: number = 1; // 导航栏当前index
...
// const oldItems: HTMLLinkElement[] = []; // 历史item数组
let totPages: number = 1; // 导航栏总标签数，初始化为1
let totItems: number = 0; // item总数，初始化0
let key: string;

export async function init(site: string) {
  key = site;
  directToSearch();
 ...
 * @returns 一个{@link HTMLLinkElement}对象
 */
export const createNewsItem = (
  ...
  const mainWrapper = document.querySelector<HTMLDivElement>('[data-element="main-wraper"]');
  ...
  mainWrapper?.removeAttribute('data-cloak');
  return item;
};
...
export const loadItems = async (page: number) => {
  ...
  const itemTemplate = document.querySelector<HTMLLinkElement>('[data-element="news-item"]'); // 获取webflow模板
  if (!itemTemplate) return;
  const itemList = itemTemplate.parentElement!; // 获取item模板父元素item list component

  const newsItems = newsData.map((data) => createNewsItem(key, data, itemTemplate)); // 使用模板初始化item数据
  // 从itemList中清除历史数据
  if (oldItems) {
 ...
 * 初始化翻页按钮
 */
export function initBeforeNextButtons() {
